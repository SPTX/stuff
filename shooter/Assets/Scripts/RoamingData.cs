...
public class RoamingData : MonoBehaviour {
	...
	void AutoLoad()
	{
		if (!File.Exists ("Save/PlayerData")) {
...
//			data = GetComponent<SaveData>();
		...
		FileStream fs = File.Open ("Save/PlayerData", FileMode.Open);
		data = (SaveData)formatter.Deserialize (fs);
		fs.Close ();

		gameObject.AddComponent (System.Type.GetType (data.TsukaiMaStraight));
		gameObject.AddComponent (System.Type.GetType (data.TsukaiMaWide));
		tsukaiMaType.AddRange (GetComponents<ShotType> ());
		tsukaiMaType [0].element = (Elements)data.TsukaiMaStraightElement;
		tsukaiMaType [1].element = (Elements)data.TsukaiMaWideElement;

//		if (data.tsukaiMaElement.Capacity >= 2) {
